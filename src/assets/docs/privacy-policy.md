...
- **Plaid Developer Credentials:** If you configure your own Plaid integration, we collect the client ID and secret you provide. These credentials are encrypted with your personal data encryption key and are never stored in plaintext.
...
- **Authenticate and Secure Your Account:** Verify your identity, manage sessions, enforce multi-factor authentication where required, and protect against unauthorized access.
...
- **Encryption at Rest:** Exchange API credentials and Plaid developer credentials are encrypted using AES-256-GCM with per-user data encryption keys (DEK). Each user's DEK is itself encrypted with a master key that is never stored in the database.
...
- **Multi-Factor Authentication (MFA):** MFA via Time-Based One-Time Password (TOTP) is available to all users. MFA is required before accessing financial integrations (Plaid Link) to provide an additional layer of protection for sensitive banking operations. MFA secrets are encrypted at rest, and recovery codes are hashed and stored securely.
...
- **Plaid Developer Credentials:** Encrypted Plaid client ID and secret are retained while your Plaid configuration is active. When you delete your Plaid configuration or close your account, these credentials are permanently deleted.
...
- **MFA Data:** TOTP secrets and recovery codes are retained while MFA is enabled. Disabling MFA triggers immediate deletion of associated secrets and recovery codes.
...
- **Consent Records:** Records of your consent to this Privacy Policy and the Terms of Service (including timestamps and policy versions) are retained for 3 years after account closure for legal compliance purposes.

For full details, please see our [Data Retention & Deletion Policy](/documentation) available within the platform.
...
- **Credential Revocation:** You may remove exchange API credentials, delete Plaid developer credentials, or disconnect funding sources at any time through the platform.
