...
export class EnvironmentService {
  private readonly apiBaseUrl = `${environment.apiUrl}/Environment`;
  ...
  async load(): Promise<void> {
    try {
      const env = await firstValueFrom(this.http.get<EgibiEnvironment>(`${this.apiBaseUrl}/get-environment`));
